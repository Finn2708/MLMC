...
void controllerSetup()
{
  ...
  DDRC &= 0b11110000; // 0 => input / 1 => leave state
  ...
  PCMSK1 = 0b00001111;      // Enable Pins PCINT8...PCINT11 (PC0...PC3) for Pin Change Interrupts
...
void controllerLoop()
{
    ...
    uint16_t ticks = getTicks(); // Read current value of TCNT1 
    uint16_t dtTick = ticks - ticksLastPID;
    if (dtTick >= cpu_freq * 1000) // ~0.001s => 1kHz control-loop  ///TODO: overflow?!?!?!?
    {
        float dt = (dtTick) / (1000000.0f * cpu_freq);
        ticksLastPID = ticks;
        ...
        float pidVal = pidCon.runController(getRPM, setRPM, dt);
